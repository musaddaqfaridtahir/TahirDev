...
export default function Header() {
  ...
  const whatsappUrl = "https://wa.me/923111122125?text=Hi%20Musaddaq,%20I%20want%20to%20discuss%20a%20new%20project!";
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Nav */}
          <nav className="hidden md:flex items-center gap-8 border border-slate-800/80 bg-slate-900/80 rounded-full px-6 py-2">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-xs font-mono font-medium text-slate-300 hover:text-cyan-400 transition-colors"
              >
                {link.name}
              </a>
            ))}
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-950 border-b border-slate-800 px-4 pt-4 pb-6 mt-3">
          <div className="flex flex-col gap-3 font-mono text-sm">
            {navLinks.map((link) => (
              <a
                ...
                className="text-slate-300 hover:text-cyan-400 py-2 border-b border-slate-900"
              >
                {link.name}
              </a>
            ))}
            <a
              href="tel:+923111122125"
              className="flex items-center gap-2 text-xs text-slate-300 py-2"
            >
